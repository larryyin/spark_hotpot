...
class mainClass(val spark: SparkSession) {
  ...
  //    val SFTP_HOST= "nj51vmasda1v.nss.vzwnet.com"
  //    val SFTP_USER = "vmasndl"
  //    val SFTP_PWD= "Summer2018@"
  //
  //    var saveFilename = ""
  //    val df = spark.sql("select * from " + saveTable)
  //    if ((filenameRoot=="customer_profiles") || (filenameRoot=="primary_locations")) {
  //      saveFilename = filenameRoot+".csv"
  //    } else {
  ...
  ////      //    .option("header", "true")
  ////      //    .save(saveFileToFTPFolder+saveFilename)
  ////
  ////      saveDfToFTP(df,SFTP_HOST,SFTP_USER,SFTP_PWD,saveFileToFTPFolder,saveFilename)
  ////    }catch{
  ////      case e:Exception=>println("Failed to save result")
  ////    }
  //  }
  ...
  def isFileHDFSSizeGT100M(filepath:String) : Boolean = {
    //val filepath = "/apps/STP/VMAS/mobility_score_us/HD/2019-04-22.parquet"
